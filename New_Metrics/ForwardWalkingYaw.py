...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu1) > 0 and len(Limu2)>0 and len(Limu3) > 0 and len(Limu4) > 0 ):
...
def getMetricsGaitNew01(Limu1, Limu2, Limu3, Limu4, plotdiagrams):
    ...
    # #Limu3-LEFT
    columnss = ['Timestamp', 'elapsed(time)', 'X(number)', 'Y(number)', 'Z(number)']
    df_Limu3 = pd.DataFrame(Limu3, columns=columnss)
    
    # #Limu4-RIGHT
    df_Limu4 = pd.DataFrame(Limu4, columns=columnss)
    # Convert the Timestamp column to datetime
    ...
    yaw_filtered = butter_lowpass_filter(euler_df_degrees['Yaw (degrees)'], cutoff2, fs2, order=5)
    ...
    metrics_data = {
        ...
        "total_metrics_head": {
            ...
            "gait": aggregated_metrics
                
        }
    }
    print(metrics_data);
    datetime_string = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"{datetime_string}_ForwardWalkingYaw_metrics.txt"
