...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu1) > 0 and len(Limu2) > 0 and len(Limu3) > 0 and len(Limu4) > 0 ):
        ...
        returnedJson = getMetricsGaitNew02(Limu1,Limu2, Limu3, Limu4 ,False) 
        return returnedJson

def getMetricsGaitNew02(Limu1,Limu2, Limu3, Limu4, plotdiagrams):
    ...
    pitch_filtered = butter_lowpass_filter(euler_df_degrees['Pitch (degrees)'], cutoff2, fs2, order=5)
    #####HEAD METRICS
    ...
    filename = f"{datetime_string}_ForwardWalkingTilt_metrics.txt"
